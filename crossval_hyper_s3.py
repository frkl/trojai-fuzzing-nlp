...
if 'score' in data.data['table_ann'].d:
    for i in range(len(data.data['table_ann']['score'])):
        ...
        v=data.data['table_ann']['score'][i].cuda();
        v=v.cuda()[:-1,:]#.contiguous()
        v=v.cuda()[:200000,:]#.contiguous()
        #v=(v-v.mean(dim=1,keepdim=True))/(v.std(dim=1,keepdim=True)+1)
        v=torch.quantile(v,torch.arange(0,1+1e-20,1/N).cuda(),dim=0)
        data.data['table_ann']['score'][i]=v.data;
