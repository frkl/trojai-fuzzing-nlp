...
ind=[i for i,v in enumerate(data.data['table_ann']['task_type']) if v==params.tag]
#data.data['table_ann']=data.data['table_ann'].select_by_index(ind);
...
if 'score' in data.data['table_ann'].d:
    for i in range(len(data.data['table_ann']['score'])):
        ...
        v=data.data['table_ann']['score'][i].cuda().float();
        #if data.data['table_ann']['task_type'][i]=='ner':
        #    print('ner found')
        #    v=-torch.exp(v);
        #v=v[:-1];
        #v=v.view(v.shape[0],-1,4).mean(dim=-2).contiguous()[:,2:3];
        v2=batch_hist(v,N,dim=0);
        #v1=torch.quantile(v,torch.arange(0,1+1e-7,1/(N-1)).cuda(),dim=0)
        v1=torch.quantile(v,generate_quantile(N).cuda(),dim=0)
        data.data['table_ann']['score'][i]=torch.cat((v1.data,v2.data),dim=0);
