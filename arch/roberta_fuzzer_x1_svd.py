...
def ridge_learn(X,Y,reg):
    ...
    try:
        ...
        U,S,V=torch.svd(X+1e-4*X.abs().mean(dim=-1,keepdim=True).mean(dim=-2,keepdim=True)*(torch.rand_like(X)-0.5));
    
    regI=reg*torch.eye(nh).to(reg.device);
    if len(X.shape)>=3:
        regI=regI.view([1]*(len(X.shape)-2)+[nh,nh]);
    
    TY=torch.matmul(U.transpose(-2,-1),Y);
    w=torch.matmul(V,(S/(S**2+reg)).unsqueeze(-1)*TY);
