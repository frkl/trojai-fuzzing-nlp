...
class regressor(nn.Module):
    def __init__(self,nh):
        ...
        self.t=MLP(self.nh,self.nh,nh,2);
    
    # X: B N K
    # Y: B N Y
    def learn(self,X,Y):
        if len(X.shape)==2:
            W=self.t(torch.Tensor(self.K,self.nh).normal_().to(X.device));
        elif len(X.shape)==3:
            W=self.t(torch.Tensor(X.shape[0],self.K,self.nh).normal_().to(X.device));
        
        X=torch.matmul(X,W.transpose(-2,-1));
