...
def ridge_learn(X,Y,reg,t=1):
    ...
    residual=(Y*Y).sum(dim=-2).view(-1,1,ny) - (w0*torch.matmul(cov,w0)).sum(dim=-2).view(-1,G,ny);
    p=F.softmax(-(residual+S.view(-1,G,1)*t),dim=-2); #(B) G Y
