            ...
            for i in range(1,l+1):
                ...
                while True:
                    ...
                    for ind in range(0,i):
                        ...
                        qy_std_avg=torch.log(qy_std.clamp(min=0.01)).mean(dim=-1); 
...
def ridge_predict(X,w,b,cov,p,w0,xmul,ymul):
    ...
    for i in range(0,ny,chunk):
        ...
        print('chkpt4 %.4f'%(time.time()-t0));
        dY0_i=Y0_i.view(-1,G,N,r-i)-Y.view(-1,1,N,ny)[:,:,:,i:r]; #B G N y
        print('chkpt5 %.4f'%(time.time()-t0));
        Y0var_i=(dY0_i**2*p.view(-1,G,1,ny)[:,:,:,i:r]).sum(dim=1); #B N ny
        print('chkpt6 %.4f'%(time.time()-t0));
        Y0var.append(Y0var_i)
