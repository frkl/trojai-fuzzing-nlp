...
def ridge_learn(X,Y,reg):
    ...
    cov_inv=torch.matmul(X.transpose(-2,-1),X)+regI; #BG nh nh
    cov=torch.inverse(cov_inv.double()) #BG nh nh
    
    a=torch.matmul(Xorig.transpose(-1,-2),Y).view(-1,nh,ny); #BG nh Y
    w0=torch.matmul(cov,a.double()); #BG nh Y
    
    S=0.5*torch.linalg.slogdet(cov_inv.double()).logabsdet; #BG
    #residual=Y.unsqueeze(0)*(Y.unsqueeze(0)-torch.matmul(X,w0)) #G N Y
    #residual=residual.sum(-2); # G Y 
    residual=(Y*Y).sum(dim=-2).view(-1,1,ny).double() - (w0*torch.matmul(cov,w0)).sum(dim=-2).view(-1,G,ny);
    p=F.softmax(-(residual+S.view(-1,G,1)),dim=-2); #(B) G Y
    
    w=(w0*p.view(-1,1,ny)).view(*Xorig.shape[:-2],G*nh,ny);
    
    return w,b,cov,p,reg

def ridge_predict(X,w,b,cov,p,reg):
    X=X.type(reg.data.dtype) #(B) N 512
    ...
    Yvar=(X*torch.matmul(X,cov)).sum(dim=-1,keepdim=True).clamp(min=0) #(B) N 1
    Ystd=Yvar**0.5;
    return Y,Ystd
...
class surrogate(nn.Module):
    def __init__(self,we,maxl=8):
        ...
        self.T=nn.Parameter(torch.Tensor(512,nh).uniform_(-1/nh**0.5,1/nh**0.5))
    ...
    def regress(self,x,y):
        ...
        e=self.embed(x); #Nxnh
        e=torch.mm(e.view(-1,512),self.T).view(*e.shape[:-1],-1);
        ...
        w_=w.transpose(-2,-1).contiguous().view(-1,w.shape[-2]); # B Y Gnh
        w_=torch.mm(w_,self.T.double().t()) #B Y 512
        w=w_.view(*w.shape[:-2],w.shape[-1],512).transpose(-2,-1).contiguous() #(B) 512 Y
        
        #cov  BG nh nh
        G=p.shape[-2];
        nh=cov.shape[-1];
        cov=cov.view(-1,G,nh,nh); #B G nh nh
        p=p.mean(-1).view(-1,G,1,1);
        cov=cov*p; #B G nh nh
        cov=cov.permute(1,0,2,3).contiguous().view(G,-1,nh);
        
        T=self.T.double().view(512,G,nh).permute(1,2,0).contiguous(); #G nh 512
        cov=torch.matmul(cov,T).view(G,-1,nh,512) #G B nh 512
        cov=cov.permute(0,1,3,2).contiguous() #G B 512 nh
        cov=cov.view(G,-1,nh);
        cov=torch.matmul(cov,T).view(G,-1,512,512).sum(0); #B 512 512
        cov=cov.view(*w.shape[:-2],512,512)
