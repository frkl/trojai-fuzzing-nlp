...
class Fuzzer():
    ...
    def find_min(self,x,y,l=8,w=None):
        ...
        ws=self.surrogate.regress(x[ind_train],y[ind_train],fast=True);
        pred_y,pred_y_std=self.surrogate.score(x[ind_test],*ws,fast=True);
        ...
        ws=self.surrogate.regress(x,y,fast=True);
        best_scores=[];
            ...
            for i in range(1,l+1):
                ...
                while True:
                    ...
                    for ind in range(0,i):
                        ...
                        qy,qy_std=self.surrogate.score(tokens_,*ws,fast=True);
                        qy_std=qy_std*s_cal;
            ...
            for i in range(1,l+1):
                ...
                while True:
                    ...
                    for ind in range(0,i):
                        ...
                        qy_std_avg=(qy_std**2).sum(dim=-1)/qy.shape[-1]/qy.shape[-1];
                        qy_std_avg=qy_std_avg**0.5;
...
def ridge_learn_fast(X,Y,reg):
    ...
    b=Y.mean(-2,keepdim=True); #b1y
    w=torch.matmul(u.transpose(-1,-2),Y-b);
    vs=v*(s/(s**2+reg_)).unsqueeze(-2);
    w=torch.matmul(vs,w);
    
    w=w.type(reg.data.dtype)
    b=b.type(reg.data.dtype)
    v=v.type(reg.data.dtype)
    s2=s2.type(reg.data.dtype)
    return w,b,v,s2

def ridge_predict_fast(X2,w,b,v,s2):
    qy=torch.matmul(X2,w)+b;
    
    e=torch.matmul(X2,v);
    s=(e**2)*s2.unsqueeze(-2);
    qy_var=s.sum(-1,keepdim=True);
    qy_std=qy_var**0.5;
...
class surrogate(nn.Module):
    ...
    def regress(self,x,y,fast=False):
        reg=torch.exp(10*self.reg)+1e-8;
        e=self.embed(x); #Nxnh
        if fast:
            w0=ridge_learn_fast(e,y,reg);
        else:
            w0=ridge_learn(e,y,reg);
        return (w0,);
    
    
    def score(self,qx,w0,fast=False):
        e=self.embed(qx);
        if fast:
            qy,qy_std=ridge_predict_fast(e,*w0); #ny
        else:
            qy,qy_std=ridge_predict(e,*w0); #ny
        
        return qy,qy_std;
    
    def forward(self,x,y,qx,fast=False):
        ws=self.regress(x,y,fast=fast);
        qy,qy_std=self.score(qx,*ws,fast=fast);
        return qy;
