    ...
    def inference(self,examples):
        with torch.no_grad():
            ...
            for batch in examples:
                ...
                ind=attention_mask.sum(dim=0).nonzero().view(-1); #all tokens in use
                input_ids=input_ids[:,ind];
                attention_mask=attention_mask[:,ind];
                token_labels=token_labels[:,ind];
                label_mask=label_mask[:,ind];
                batch={'input_ids':input_ids,'attention_mask':attention_mask,'token_labels':token_labels,'label_mask':label_mask}
                ...
                for i in range(logits.shape[0]):
                    ind=batch['label_mask'][i].view(-1).nonzero().long().view(-1);
                    ind=ind*logits.shape[2]+batch['token_labels'][i][ind];
                    s=logits[i].view(-1)[ind];
                    outputs.append(s);
        
        
        #outputs=torch.cat(outputs,dim=0);
        #outputs=torch.log(outputs.abs()+1e-9)
        return outputs
