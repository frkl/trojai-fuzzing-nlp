...
class new():
    ...
    def __init__(self,maxl=8,**kwargs):
        ...
        self.maxl=maxl;
        self.surrogate=surrogate(nhword=nhword,N=self.vocab_size,maxl=maxl,**kwargs).cuda();
...
def ridge_predict(X,w0,cov,p,training=False):
    ...
    Y0var=((dY0**2)*p.view(-1,G,1,ny)).sum(dim=-3).view(-1,N,ny).clamp(min=1e-12); # B N ny
    Ystd=Y0var**0.5;
