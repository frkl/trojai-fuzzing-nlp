...
def ridge_learn(X,Y,reg,t,bias,wz,training=False):
    ...
    if training:
        ...
        #bias.data=bias.data*0.99+bias_i.data*0.01;
        pass
        #logp=F.log_softmax(-0.5*(residual+S.view(-1,G,1))-bias_i.view(1,G,1),dim=-2); #B G Y
    else:
        logp=F.log_softmax(-0.5*(residual+S.view(-1,G,1))-bias.data.view(1,G,1),dim=-2); #B G Y
...
def ridge_predict(X,b,xmul,ymul,w0,cov,p):
...
class surrogate(nn.Module):
    def __init__(self,we=None,maxl=8,N=None,nhword=None,pretrained=False,**kwargs):
