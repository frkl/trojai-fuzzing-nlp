...
def ridge_learn(X,Y,reg,t,bias,wz,training=False):
    ...
    cov=torch.inverse(cov_inv).type(reg.data.dtype) #BG nh nh
    a=torch.matmul(GX.transpose(-1,-2),GY); #BG nh ny
    w0=t*torch.matmul(cov,a); #BG nh ny
    
    #Calculate mixture weights
    S=torch.linalg.slogdet(cov_inv).logabsdet.type(reg.data.dtype); #BG
    #Residual method 1
    residual=t*(GY*GY).sum(dim=-2).view(-1,G,ny) - (w0*torch.matmul(cov,w0)).sum(dim=-2).view(-1,G,ny); # B G Y
    logp=-0.5*(residual+S.view(-1,G,1));
    if training:
        #bias_i=logp.permute(1,0,2).contiguous().view(G,-1);
        #bias_i=bias_i.quantile(1-1/G,dim=1);
        #bias_i=bias_i-bias_i.mean();
        
        bias_i=logp.mean(dim=-1).mean(dim=0);
        bias.data=bias.data*0.95+bias_i.data*0.05;
        
        logp=F.log_softmax(-0.5*(residual+S.view(-1,G,1))-bias_i.view(1,G,1),dim=-2); #B G Y
    else:
        logp=F.log_softmax(-0.5*(residual+S.view(-1,G,1))-bias.view(1,G,1),dim=-2); #B G Y
    
    p=F.softmax(logp,dim=-2); #B G Y
    
    #print(S.view(-1,G)[0,:].view(-1).tolist())
    #print(residual[0,:,0].tolist())
    
    #Produce a single weight vector for fast inference? nay
    
    w0=w0.type(reg.data.dtype);
    b=b.type(reg.data.dtype);
    cov=cov.type(reg.data.dtype);
    p=p.type(reg.data.dtype);
    return b,xmul,ymul,w0,cov,p

def ridge_predict(X,b,xmul,ymul,w0,cov,p,training=False):
    ...
    if training:
        Ystd=torch.zeros_like(Y)+1;
    else:
        dY0=GY.view(-1,G,N,ny)-Y.view(-1,1,N,ny);
        Y0var=((dY0**2)*p.view(-1,G,1,ny)).sum(dim=-3).view(-1,N,ny).clamp(min=1e-12);
        
        
        Yvar=(torch.matmul(GX,cov)*GX).sum(dim=-1).view(-1,G,N) #(BG N,1) #var of y within each Gaussian mode
        Yvar=(Yvar.view(-1,G,N,1)*p.view(-1,G,1,ny)).sum(-3).clamp(min=1e-12) #B G ny
...
class surrogate(nn.Module):
    ...
    def regress(self,x,y):
        ...
        e=self.embed(x); #Nxnh
        if self.training:
            w0=ridge_learn(e,y,reg,t,self.bias,self.w0,training=self.training);
        else:
            #only tune temperature t
            w0=ridge_learn(e.data,y,reg,t,self.bias,self.w0,training=self.training);
