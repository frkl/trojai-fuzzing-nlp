...
def ridge_learn(X,Y,reg,t=1):
    ...
    a=torch.matmul(X.transpose(-1,-2),Y); #bhy
    cov=torch.inverse(A.double()) #bhh
    w=torch.matmul(cov,a); #bhy
    
    w=w.type(reg.data.dtype);
    b=b.type(reg.data.dtype);
    cov=cov.type(reg.data.dtype);
    return w,b,cov,xmul,ymul

def ridge_predict(X,w,b,cov,xmul,ymul):
    #Convert floats to double
    X=X.type(w.data.dtype) #bnh
    #print(X.shape,xmul.shape)
    X=X/xmul;
    
    Y=torch.matmul(X,w)
    
    Ystd=(torch.matmul(X,cov)*X).sum(dim=-1,keepdim=True).clamp(min=0);
    sz=[1 for i in Y.shape];
    sz[-1]=Y.shape[-1];
    Ystd=Ystd.repeat(*sz);
