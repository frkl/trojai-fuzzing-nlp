...
if 'score' in data.data['table_ann'].d:
    ...
    for i in range(len(data.data['table_ann']['score'])):
        ...
        x=x/(x.std(dim=0,keepdim=True)+1);
        u,s,v=torch.svd(x);
        s=torch.log((s**2).clamp(min=1e-20))
        s=s.view(1,-1).sort(dim=1)[0];
        eigs.append(s.view(-1).float());
    
    data.data['table_ann'].d['eigs']=eigs;
